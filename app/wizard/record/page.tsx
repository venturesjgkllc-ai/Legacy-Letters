...
export default function RecordStep() {
  ...
  const [audioUrl, setAudioUrl] = useState<string | null>(state.audioUrl ?? null);
  ...
  return (
    <div className="animate-fade-in-up">
      ...
      {/* Permission denied */}
      {permissionDenied && (
        <div role="alert" aria-live="assertive" className="bg-red-50 border border-red-200 rounded-2xl p-6 mb-6 text-center">
      ...
      {/* Recording UI */}
      <div className="card-warm p-8 text-center mb-6">
        {/* Waveform */}
        <div
        ...
        {/* Timer */}
        <div
          className="font-serif text-5xl text-ink mb-2 tabular-nums"
          aria-live="polite"
          aria-atomic="true"
        ...
        <p className="font-sans text-sm text-sepia-400 mb-6" aria-live="polite">
        ...
        <div className="w-full bg-sepia-100 rounded-full h-2 mb-8 overflow-hidden">
          <div
            ...
            aria-label={`Recording progress: ${formatTime(seconds)} of ${formatTime(MAX_SECONDS)}`}
          />
        </div>
        ...
        {status === "idle" && (
          <button
            ...
            className="w-24 h-24 rounded-full bg-red-600 hover:bg-red-500 text-white shadow-warm-lg transition-all duration-200 hover:scale-105 focus:outline-none focus:ring-4 focus:ring-red-300 mx-auto flex items-center justify-center text-4xl"
        ...
        {status === "recording" && (
          <button
            ...
            className="w-24 h-24 rounded-full bg-red-600 text-white shadow-warm-lg animate-record-pulse transition-all duration-200 focus:outline-none focus:ring-4 focus:ring-red-300 mx-auto flex items-center justify-center font-sans font-500 text-lg"
      ...
      {/* ── AUDIO PLAYBACK — shows immediately after recording stops ── */}
      {status === "done" && audioUrl && (
        <div className="card-warm p-6 mb-6">
          <p className="font-sans text-sm font-500 text-sepia-600 mb-3 text-center">
          ...
          <audio
            ...
            aria-label="Your recording — listen back before submitting"
            style={{ minHeight: "54px" }}
          />
          {tooShort && (
            <p className="font-sans text-sm text-amber-700 mt-3 text-center" role="alert">
              Your recording is a little short. We recommend at least 1 minute for a meaningful letter.
            </p>
          )}
          <div className="flex gap-3 mt-4 justify-center">
            <button
              ...
              aria-label="Discard this recording and record again"
            >
              🔄 Re-record
            </button>
          </div>
      ...
      <div className="flex gap-4 mt-2">
        ...
        <button
          ...
          style={{ flex: 2 }}
          aria-label="Create my letter from this recording"
          aria-disabled={status !== "done" || tooShort}
        >
          Create my letter →
        </button>
      </div>
